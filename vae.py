...
class VAE96(nn.Module):
  ...
  def __init__(self, in_ab, in_l, nf, ld, ks, do=0.7):
    ...
    self.fc_cond_ab = torch.nn.Linear(64 * nf, ld * 2)
  ...
  def forward(self, color, greylevel):
    ...
    return mu, logvar, color_out
    """
    mu_c, logvar_c, sc_feat32, sc_feat16, sc_feat8, sc_feat4 = self.cond_encoder(greylevel)
    mu, logvar = self.encode(color)
    stddev = torch.sqrt(torch.exp(logvar))
    sample = torch.randn(stddev.shape, device=stddev.device)
    z = torch.add(mu, torch.mul(sample, stddev))
    color_out = self.decoder(z, sc_feat32, sc_feat16, sc_feat8, sc_feat4)
    return mu, logvar, color_out, mu_c, logvar_c


class VAELAB(nn.Module):
    def __init__(self, in_ab, in_l, nf, ld, ks, do=0.7):
        super(VAELAB, self).__init__()
        self.ld = ld
        self.relu = torch.nn.ReLU()
        self.tanh = torch.nn.Tanh()
        self.dropout = torch.nn.Dropout(do)
        self.fc = torch.nn.Linear(64 * nf, ld * 2)

        self.enc_conv1 = torch.nn.Conv2d(in_l, nf, ks, stride=2, padding=17, dilation=1)
        self.enc_bn1 = torch.nn.BatchNorm2d(nf)
        self.enc_conv2 = torch.nn.Conv2d(nf, 2 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn2 = torch.nn.BatchNorm2d(2 * nf)
        self.enc_conv3 = torch.nn.Conv2d(2 * nf, 4 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn3 = torch.nn.BatchNorm2d(4 * nf)
        self.enc_conv4 = torch.nn.Conv2d(4 * nf, 8 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn4 = torch.nn.BatchNorm2d(8 * nf)
        self.enc_conv5 = torch.nn.Conv2d(8 * nf, 16 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn5 = torch.nn.BatchNorm2d(16 * nf)
        self.enc_conv6 = torch.nn.Conv2d(16 * nf, 32 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn6 = torch.nn.BatchNorm2d(32 * nf)
        self.enc_conv7 = torch.nn.Conv2d(32 * nf, 64 * nf, ks, stride=2, padding=1, dilation=1)
        self.enc_bn7 = torch.nn.BatchNorm2d(64 * nf)
    ...
    def encode(self, x):
        ...
        sc_feat32 = self.enc_bn2(self.enc_conv2(x))
        x = self.relu(sc_feat32)
        sc_feat16 = self.enc_bn3(self.enc_conv3(x))
        x = self.relu(sc_feat16)
        sc_feat8 = self.enc_bn4(self.enc_conv4(x))
        x = self.relu(sc_feat8)
        sc_feat4 = self.enc_bn5(self.enc_conv5(x))
        x = self.relu(sc_feat4)
    ...
    def decode(self, z, sc_feat32, sc_feat16, sc_feat8, sc_feat4):
        z = z.reshape(z.shape[0], self.ld, 1, 1)
        z = self.relu(self.dec_bn1(self.dec_conv1(z)))
        z = self.relu(self.dec_bn2(self.dec_conv2(z)))
        z = torch.cat((z, sc_feat4), 1)
    ...
    def forward(self, greylevel):
        ...
        return mu, logvar, color_out
        """
        mu, logvar, sc_feat32, sc_feat16, sc_feat8, sc_feat4 = self.encode(greylevel)
        stddev = torch.sqrt(torch.exp(logvar))
        sample = torch.randn(stddev.shape, device=stddev.device)
        z = torch.add(mu, torch.mul(sample, stddev))
        color_out = self.decode(z, sc_feat32, sc_feat16, sc_feat8, sc_feat4)
        return mu, logvar, color_out
