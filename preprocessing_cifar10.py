...
if __name__=="__main__":
    ...
    trainset = torchvision.datasets.CIFAR10(root="../datasets/cifar10/train", train=True, download=False)
    testset = torchvision.datasets.CIFAR10(root="../datasets/cifar10/test", train=False, download=False)
    if not args.debug:
        train_tensor = torch.tensor(trainset.data, device="cpu").transpose(-1, 1).transpose(-1, -2)
        test_tensor = torch.tensor(testset.data, device="cpu").transpose(-1, 1).transpose(-1, -2)
    else:
        train_tensor = torch.tensor(trainset.data[:N], device="cpu").transpose(-1, 1).transpose(-1, -2)
        test_tensor = torch.tensor(testset.data[:N], device="cpu").transpose(-1, 1).transpose(-1, -2)
