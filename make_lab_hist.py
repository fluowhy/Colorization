...
if __name__ == "__main__":
    train_lab = np.load("../datasets/stl10/train_lab.npy")
    val_lab = np.load("../datasets/stl10/val_lab.npy")
    ...
    hist, edges = np.histogramdd(np.hstack((l, a, b)), bins=(np.arange(102) - 0.5, np.arange(256) - 0.5, np.arange(256) - 0.5))

    train_hist_values = make_lab_hist_image_set(hist, train_lab)
    val_hist_values = make_lab_hist_image_set(hist, val_lab)
    np.save("../datasets/stl10/train_hist_values", train_hist_values)
    np.save("../datasets/stl10/val_hist_values", val_hist_values)
